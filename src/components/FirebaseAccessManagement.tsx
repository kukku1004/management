...
export default function FirebaseAccessManagement() {
  ...
  async function save() {
    ...
    try {
      const data = { role: form.role, teamIds: form.teamIds, canViewOtherTeams: form.role === 'admin' || (form.role === 'leader' && form.canViewOtherTeams), updatedAt: serverTimestamp() }
  ...
  return <section className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 grid gap-4 rounded-lg border border-gray-200 p-5 md:grid-cols-2">
      ...
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.canViewOtherTeams} onChange={e => setForm(v => ({ ...v, canViewOtherTeams: e.target.checked }))} disabled={form.role !== 'leader'} />팀장에게 다른 팀 조회 허용</label><div className="flex justify-end"><button onClick={() => void save()} disabled={busy} className="ui-button ui-button-primary px-5">{busy ? '저장 중…' : '초대·권한 저장'}</button></div>{message && <p className="text-sm text-gray-700 md:col-span-2">{message}</p>}
    </div>
