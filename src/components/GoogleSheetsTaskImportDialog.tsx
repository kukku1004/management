...
export default function GoogleSheetsTaskImportDialog({ tasks, onImport, onClose, embedded = false }: Props) {
  ...
  const content = <div className={embedded ? 'w-full min-w-0' : 'h-[80dvh] w-[80vw] min-w-0 overflow-y-auto overflow-x-hidden rounded-xl border border-gray-200 bg-white p-6 shadow-xl'}>
    ...
    {preview && <div className="mt-5 min-w-0 overflow-hidden rounded-lg border border-gray-200">
      <div className="border-b border-gray-200 bg-gray-50 p-3"><label className="flex max-w-sm items-center gap-3 text-sm font-medium text-gray-700"><span className="shrink-0">분야</span><select value={activeLevel1} onChange={(event) => setActiveLevel1(event.target.value)} className="ui-field ui-field-sm min-w-0 flex-1">{level1s.map((level1) => <option key={level1} value={level1}>{level1} ({preview.groups.filter((group) => group.level1 === level1).length})</option>)}</select></label></div>
    ...
    <div className="mt-5 flex items-center justify-between"><p className="text-sm text-gray-500">선택한 L2 상위과제 {selected.size}개</p><div className="flex gap-2">{!embedded && <button type="button" onClick={onClose} className="ui-button ui-button-secondary">취소</button>}<button type="button" disabled={!preview || selected.size === 0} onClick={finish} className="ui-button ui-button-primary">선택 과제 가져오기</button></div></div>
  </div>
