...
export default function TaskManagement() {
  ...
  return (
    ...
    <div className="ui-page">
      ...
      {hasTasks ? (
      <div>
      {(sourceGroups.length > 0 || (activeView === 'register' && canManage)) && <div className="mb-3 flex items-start justify-between gap-3 border-b border-gray-200"><div className="flex min-w-0 flex-wrap gap-1" role="tablist" aria-label="L1 분야">{['전체', ...sourceGroups].map((group) => { const count = group === '전체' ? rootTasks.length : rootTasks.filter((task) => task.sourceGroup === group).length; return <button key={group} type="button" role="tab" aria-selected={activeSourceGroup === group} onClick={() => { setActiveSourceGroup(group); setSelectedTaskIds(new Set()) }} className={`ui-tab rounded-b-none ${activeSourceGroup === group ? 'ui-tab-active' : ''}`}>{group} <span className="ml-1 text-xs opacity-60">{count}</span></button> })}</div>{activeView === 'register' && canManage && <button type="button" onClick={() => openTaskForm('L2')} className="ui-button ui-button-primary ui-button-sm mb-2 shrink-0">+ 상위과제 추가</button>}</div>}
